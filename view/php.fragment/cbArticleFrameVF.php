...
trait cbArticleFrameVF
{
  ...
  protected function pageNumbers()
  {
       ...
       )
    {
      ...
      if ($page > 0) {
        $erg .= '<a class="cbPrevPage" href="'.$this->linker->cbArticleLink($this->ep, $this->mod, $this->hook, $this->data['articleBox'], $this->data['articleName'], ($page-1)).'">&laquo;</a>';
      }
      $erg .= '<div class="cbPages">';
      for($i = 0; $i < $numOfPages; $i++) {
        if ($page != $i) {
          $erg .= '<a href="'.$this->linker->cbArticleLink($this->ep, $this->mod, $this->hook, $this->data['articleBox'], $this->data['articleName'], $i).'">'.($i+1).'</a>';
        } else {
      ...
      if (($page + 1) < $numOfPages) {
        $erg .= '<a class="cbNextPage" href="'.$this->linker->cbArticleLink($this->ep, $this->mod, $this->hook, $this->data['articleBox'], $this->data['articleName'], ($page+1)).'">&raquo;</a>';
      }
      $erg .= '</div>';
